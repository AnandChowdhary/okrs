...
export async function summarize() {
  ...
  let summary = api.years
    ...
    .map(
      (quarter, index) => `${
        index === 0
          ? `## 📈 Current OKRs – Q${quarter.name} ${quarter.year}\n\n`
          : index === 1
          ? "## ✅ Past OKRs\n\n"
          : ""
      }${
        index === 0
          ? "<div>"
          : `<details>\n  <summary>Q${quarter.name} ${quarter.year}</summary>`
      }
  <table>
    <thead>
      <tr>
  ...
  ${quarter.objectives
    .map(
      (objective) => `    <tr>
        ...
        <td><strong>${objective.progress * 100}%</strong></td>
      </tr>
  ${objective.key_results
    .map(
      (keyResult) => `    <tr>
...
<!-- end autogenerated OKR summary -->${
      README.split("<!-- end autogenerated OKR summary -->")[1]
    }`
  );
}
